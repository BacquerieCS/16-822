...
def pure_metric_rectification(points: np.ndarray) -> np.ndarray:
    """
    Computes the metric rectification matrix for the given homogeneous `points`
    using only orthogonal lines.
    """
    lines: np.ndarray = get_lines(points)
    ...
    j: int = 0
    for i in range(0, len(lines), 2):
        l: np.ndarray = lines[i]
        m: np.ndarray = lines[i + 1]
        row = (
            l[0] * m[0],
            (l[0] * m[1] + l[1] * m[0]) / 2,
            l[1] * m[1],
    ...
    a: np.ndarray = np.array(a, np.float64)
    # Reconstruction of the circular points.
    _, _, sol = np.linalg.svd(a)
    s: np.ndarray = sol[-1]
